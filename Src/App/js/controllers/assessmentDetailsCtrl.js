...
(function () {
    ...
    angular
        .module('app')
        .controller('assessmentDetailsCtrl', ['$rootScope', '$scope', '$routeParams', '$location', '$filter', 'storage', 'config', 'util', 'AssessmentService', 'LookupService',
            function ($rootScope, $scope, $routeParams, $location, $filter, storage, config, util, AssessmentService, LookupService) {
                ...
                function _getOverallRiskReport(id) {
                    AssessmentService.getRiskScoreById(id).then(function (result) {
                ...
                function _setPermission() {
                    ...
                    if (($scope.assessmentDetail.approvalStatus === config.APPROVAL_STATUS.AWAITING_BU_FUNCTIONAL_APPROVAL.value && $rootScope.user.role === config.ROLES.BU_FUNCTIONAL_APPROVER_ROLE_NAME) ||
                        ...
                        ($rootScope.user.role === config.ROLES.ADMIN_ROLE_NAME &&
                            ($scope.assessmentDetail.approvalStatus === config.APPROVAL_STATUS.AWAITING_BU_FUNCTIONAL_APPROVAL.value ||
                                $scope.assessmentDetail.approvalStatus === config.APPROVAL_STATUS.AWAITING_BU_RISK_MANAGEMENT_APPROVAL.value ||
                                $scope.assessmentDetail.approvalStatus === config.APPROVAL_STATUS.AWAITING_DIVISIONAL_RISK_MANAGEMENT_APPROVAL.value))) {
                ...
                function _setClassByStatus() {
                    switch ($scope.assessmentDetail.approvalStatus) {
                        ...
                        case config.APPROVAL_STATUS.AWAITING_DIVISIONAL_RISK_MANAGEMENT_APPROVAL.value:
                            $scope.panelClass = $rootScope.isApprover || $rootScope.user.role == config.ROLES.ADMIN_ROLE_NAME ? 'awaiting-approver' : '';
                    ...
                    if (assessment.functionalAreaProcessAssessments) {
                        ...
                        angular.forEach(assessment.functionalAreaProcessAssessments, function (functionalAssessment) {
                            if (!functionalAreaError) {
                                ...
                                if (functionalAssessment.controlAssessments && functionalAreaError == false) { // jshint ignore:line
