 ...
 * The service helper
 *
 * Changes in 1.1 during 72H! JOHN HANCOCK - PROJECT COEUS USERS AND ROLES MANAGEMENT challenge:
 * - added 'makeRequestWithStatus' method
 *
    ...
    angular
        ...
        .factory('common', ['$http', '$q', '$log', 'storage', 'config', 'util', '$injector', 'FileSaver', 'Blob',
            function ($http, $q, $log, storage, config, util, $injector, FileSaver, Blob) {
                ...
                /**
                 * Make an HTTP request and add access token which returns status code when error
                 * @param {Object} options the options for $http call
                 * @returns {promise} the promise with result
                 */
                common.makeRequestWithStatus = function (options) {
                    var deferred = $q.defer();
                    ...
                    $http(_createRequestOption(options))
                        ...
                        .error(function (data, status) {
                            // data will be null if backend service is not available.
                            var result={
                                error:data,
                                status:status
                            };
