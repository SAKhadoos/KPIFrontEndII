...
namespace JohnHancock.Common
{
    /// <summary>
    /// This class is the common helper class for this project.
    /// </summary>
    ///
    /// <threadsafety>
    ...
    internal static class CommonHelper
    {
        ...
        {
            ValidateConfigPropertyNotNullOrEmpty(property, nameof(property));
            if (property.Keys.Any(string.IsNullOrWhiteSpace))
            {
                throw new ConfigurationException(
                    $"Configurable property {property} should not contain empty keys.");
            }
            if (property.Values.Any(string.IsNullOrWhiteSpace))
            {
                throw new ConfigurationException(
        ...
        /// <summary>
        /// Validates that <paramref name="param"/> is not <c>null</c> or empty array of positive number.
        /// </summary>
        ///
        /// <param name="param">The parameter to validate.</param>
        /// <param name="paramName">The name of the parameter.</param>
        ...
        /// If <paramref name="param"/> is empty or contains any non positive numbers.</exception>
        internal static void ValidatePositiveArray(long[] param, string paramName)
        {
            ValidateArgumentNotNullOrEmpty(param, paramName);

            if (param.Any(id => id <= 0))
            {
                throw new ArgumentException($"The {paramName} should contain only positive number.");
            }
        }
        ...
        /// <exception cref="ArgumentException">
        /// If <paramref name="param"/> is empty or contains any non positive numbers.</exception>
        internal static void ValidatePositiveArray(int[] param, string paramName)
        {
            ValidateArgumentNotNullOrEmpty(param, paramName);
        ...
        ///
        /// <exception cref="ArgumentException">
        /// If <paramref name="param"/> is null or not positive number.</exception>
        internal static void ValidateNotNullPositive(decimal? param, string paramName)
        {
            if (!param.HasValue)
            {
                throw new ServiceException($"{paramName} should not be null.");
            }
            if (param.HasValue && param.Value <= 0)
            {
                throw new ServiceException($"{paramName} should be positive.");
            }
        }
        ...
        internal static string GetJtokenValue(JToken token)
        {
