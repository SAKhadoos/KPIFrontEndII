...
namespace JohnHancock.ProjectCoeus.Services.Impl
{
    ...
    /// - removed methods 'ResolveEntities' and 'ResolveChildEntity'
    /// Changes in 1.4 during JOHN HANCOCK - PROJECT COEUS ADMIN BACKEND ASSEMBLY
    ///
    /// Changed in 1.5 during JOHN HANCOCK - PROJECT COEUS ADMIN FRONTEND ASSEMBLY PART 2
    /// </remarks>
    ///
    /// <author>LOY, NightWolf, veshu, albertwang, TCSASSEMBLER</author>
    /// <version>1.5</version>
    /// <copyright>Copyright (c) 2016, TopCoder, Inc. All rights reserved.</copyright>
    public class AssessmentService : BasePersistenceService<CustomDbContext>, IAssessmentService
    {
        /// <summary>
        ...
        public override void CheckConfiguration()
        {
            ...
            CommonHelper.ValidateConfigPropertyNotNull(RiskScoreCalculator, nameof(RiskScoreCalculator));
        ...
        public void ExportAssessment(Assessment assessment, ExportFormat format, Stream outputStream)
        {
            ProcessWithDb(db =>
            {
                CommonHelper.ValidateArgumentNotNull(assessment, nameof(assessment));
                CommonHelper.ValidateArgumentNotNull(outputStream, nameof(outputStream));
        ...
        /// <summary>
        /// This method is used to get assessment risk score.
        /// </summary>
        ///
        /// <param name="assessment">The assessment.</param>
        ///
        ...
        ///
        /// <exception cref="ArgumentNullException">
        /// If the <paramref name="assessment"/> is <c>null</c>.
        /// </exception>
        /// <exception cref="PersistenceException">
        ...
        /// If any other errors occur while performing this operation.
        /// </exception>
        public RiskScore GetRiskScore(Assessment assessment)
        {
            return Logger.Process(() =>
            {
                ...
                {
                    CategoryInherentRiskScores = new List<CategoryRiskScore>(),
                    CategoryResidualRiskScores = new List<CategoryRiskScore>()
                };

                // Calculation for FunctionalAreaProcessAssessments
                if (assessment.FunctionalAreaProcessAssessments.Any())
                {
                    // get category
                    var category = assessment.FunctionalAreaProcessAssessments[0].Category;

                    // Group by core process
                    var fPAgroupByCoreProcess =
                        assessment.FunctionalAreaProcessAssessments.Where(p => p.CoreProcess != null)
                            .GroupBy(r => r.CoreProcess.Id)
                            .ToDictionary(g => g.Key,
                                g => (IList<ProcessControlAssessment>)g.Cast<ProcessControlAssessment>().ToList());

                    // for additional core process group by additional core process
                    var additionalGroupByAdditionalProcess =
                        assessment.FunctionalAreaProcessAssessments.Where(p => p.CoreProcess == null)
                            .GroupBy(r => r.AdditionalCoreProcess)
                            .ToDictionary(g => g.Key,
                                g => (IList<ProcessControlAssessment>)g.Cast<ProcessControlAssessment>().ToList());

                    var maxKey = fPAgroupByCoreProcess.Keys.Count > 0 ? fPAgroupByCoreProcess.Keys.Max() : 0;
                    foreach (var additionalGroup in additionalGroupByAdditionalProcess)
                    {
                        maxKey++;
                        fPAgroupByCoreProcess.Add(maxKey, additionalGroup.Value);
                    }

                    // compute
                    ComputeRiskScore(riskScore, category, fPAgroupByCoreProcess);
                }

                // Calculation for other process assessments
                if (assessment.ProcessRiskAssessments.Any())
                {
                    // Group by category
                    var groupByCategory = assessment.ProcessRiskAssessments.GroupBy(r => r.Category.Id)
                        .ToDictionary(g => g.Key, g => g.ToList());

                    foreach (var categoryGroup in groupByCategory)
                    {
                        // get category
                        var category = categoryGroup.Value[0].Category;

                        // Group by process risk
                        var groupByProcessRisk =
                            categoryGroup.Value.Where(p => p.ProcessRisk != null)
                                .GroupBy(r => r.ProcessRisk.Id)
                                .ToDictionary(g => g.Key,
                                    g => (IList<ProcessControlAssessment>)g.Cast<ProcessControlAssessment>().ToList());

                        // for additional process risk group by additional process risk
                        var additionalGroupByAdditionalProcess =
                            categoryGroup.Value.Where(
                                p => p.ProcessRisk == null && string.IsNullOrEmpty(p.AdditionalProcess))
                                .GroupBy(r => r.AdditionalProcess)
                                .ToDictionary(g => g.Key,
                                    g => (IList<ProcessControlAssessment>)g.Cast<ProcessControlAssessment>().ToList());

                        var maxKey = groupByProcessRisk.Keys.Count > 0 ? groupByProcessRisk.Keys.Max() : 0;
                        foreach (var additionalGroup in additionalGroupByAdditionalProcess)
                        {
                            maxKey++;
                            groupByProcessRisk.Add(maxKey, additionalGroup.Value);
                        }

                        // compute
                        ComputeRiskScore(riskScore, category, groupByProcessRisk);
                    }
                }

                // calculate overall inherent and residual risk score from each category score and its category weight
                riskScore.CategoryInherentRiskScores.ForEach(
                    r => riskScore.OverallInherentRiskScore += r.Score * (r.Category.Weight / 100));

                riskScore.CategoryResidualRiskScores.ForEach(
                    r => riskScore.OverallResidualRiskScore += r.Score * (r.Category.Weight / 100));

                return riskScore;
            }, "calculating the risk score", parameters: assessment);
        }

        #region private methods

        /// <summary>
        /// Computes the risk score for a category and process control assessments.
        /// </summary>
        /// <param name="riskScore">The risk score result container.</param>
        /// <param name="category">The risk category.</param>
        /// <param name="pcaGroup">The process control assessments for the given category.</param>
        private static void ComputeRiskScore(RiskScore riskScore, Category category,
            Dictionary<long, IList<ProcessControlAssessment>> pcaGroup)
        {
            var categoryIRS = new CategoryRiskScore() { Category = category, Score = 0 };
            var categoryRRS = new CategoryRiskScore() { Category = category, Score = 0 };

            decimal totalCoreProcessInherentRiskScore = 0;
            decimal totalCoreProcessResidualRiskScore = 0;

            // for each CoreProcess/ProcessRisk
            foreach (var pcaKeyValuePair in pcaGroup)
            {
                var pcas = pcaKeyValuePair.Value;

                decimal individualCoreProcessInherentRiskScore = 0;
                decimal individualCoreProcessResidualRiskScore = 0;

                decimal sumOfAllSubProcessInherentRiskScore = 0;
                decimal sumOfAllSubProcessResidualRiskScore = 0;

                if (pcas != null)
                {
                    pcas.ForEach(p =>
                    {
                        // sum of all Sub-Process Inherent risk scores
                        sumOfAllSubProcessInherentRiskScore +=
                            p.LikelihoodOfOccurrence?.Value ?? 1 * p.RiskExposure?.Value ?? 1;

                        // sum of all Sub-Process Residual risk scores
                        sumOfAllSubProcessResidualRiskScore +=
                            p.LikelihoodOfOccurrence?.Value ?? 1 * p.RiskExposure?.Value ?? 1 *
                                GetAverageKeyControlsMaturity(p);
                    });
                }

                // for each sub processes
                foreach (var pca in pcas)
                {
                    // if Likelihood of Occurrence/RiskExposure is null then
                    // value of should be "1" for the sake of the calculation
                    var locValue = pca.LikelihoodOfOccurrence?.Value ?? 1;
                    var riskExposureValue = pca.RiskExposure?.Value ?? 1;

                    // Sub process inherent Risk score
                    var subProcessInherentRiskScore = locValue * riskExposureValue;

                    // Sub process Inherent Risk Score Weight is individual Sub-Process inherent risk score
                    // divided by the sum of all Sub-Process inherent risk scores
                    decimal subProcessInherentRiskScoreWeight = 0;
                    if (sumOfAllSubProcessInherentRiskScore > 0)
                    {
                        subProcessInherentRiskScoreWeight =
                            subProcessInherentRiskScore / sumOfAllSubProcessInherentRiskScore;
                    }

                    // Inherent risk score of individual Core process
                    // is sum of (Sub process Inherent Risk Score X Sub process Inherent Risk Weight)
                    individualCoreProcessInherentRiskScore +=
                        subProcessInherentRiskScore * subProcessInherentRiskScoreWeight;

                    decimal averageKeyControlsMaturity = GetAverageKeyControlsMaturity(pca);

                    // Residual Risk Score
                    var subProcessResidualRiskScore = locValue * riskExposureValue * averageKeyControlsMaturity;

                    // Sub process Residual Risk Score Weight is individual Sub-Process Residual risk score
                    // divided by the sum of all Sub-Process Residual risk scores
                    decimal subProcessResidualRiskScoreWeight = 0;
                    if (sumOfAllSubProcessResidualRiskScore > 0)
                    {
                        subProcessResidualRiskScoreWeight =
                            subProcessResidualRiskScore / sumOfAllSubProcessResidualRiskScore;
                    }

                    // Inherent risk score of individual Core process
                    // is sum of (Sub process Inherent Risk Score X Sub process Inherent Risk Weight)
                    individualCoreProcessResidualRiskScore +=
                        subProcessResidualRiskScore * subProcessResidualRiskScoreWeight;
                }

                // total core process inherent risk is sum of inherent risk score of individual core process
                totalCoreProcessInherentRiskScore += individualCoreProcessInherentRiskScore;

                // total core process residual risk is sum of residual risk score of individual core process
                totalCoreProcessResidualRiskScore += individualCoreProcessResidualRiskScore;
            }

            if (pcaGroup.Count > 0)
            {
                // category inherent risk score is total inherent risk score of all core processes
                // divided by number of core-processes
                categoryIRS.Score = totalCoreProcessInherentRiskScore / pcaGroup.Count;

                // category residual risk score is total residual risk score of all core processes
                // divided by number of core-processes
                categoryRRS.Score = totalCoreProcessResidualRiskScore / pcaGroup.Count;
            }

            riskScore.CategoryInherentRiskScores.Add(categoryIRS);
            riskScore.CategoryResidualRiskScores.Add(categoryRRS);
        }

        private static decimal GetAverageKeyControlsMaturity(ProcessControlAssessment pca)
        {
            decimal averageKeyControlsMaturity = 0;
            // find the sum of key controls maturity value of each
            if (pca.ControlAssessments != null)
            {
                decimal totalKeyControlsMaturityValue = 0;
                pca.ControlAssessments.ForEach(r =>
                {
                    // take value as 1 if key control maturity is not selected or
                    // for the case of "No controls in Place" (in this case key control maturity will be null)
                    totalKeyControlsMaturityValue += r.KeyControlsMaturity?.Value ?? 1;
                });

                if (pca.ControlAssessments.Count > 0)
                {
                    averageKeyControlsMaturity = totalKeyControlsMaturityValue / pca.ControlAssessments.Count;
                }
            }

            return averageKeyControlsMaturity > 0 ? averageKeyControlsMaturity : 1;
        }
        ...
        private static IQueryable<Assessment> IncludeNavigationProperties(IQueryable<Assessment> query)
        {
            return query.Include(x => x.AssessmentStatus)
                ...
                .Include(x => x.FunctionalAreaProcessAssessments.Select(
                    p => p.ControlAssessments.Select(c => c.ControlDesigns)))
                .Include(x => x.FunctionalAreaProcessAssessments.Select(
                    p => p.ControlAssessments.Select(c => c.ControlFrequency)))
                .Include(x => x.FunctionalAreaProcessAssessments.Select(
                    p => p.ControlAssessments.Select(c => c.ControlTriggers)))
                .Include(x => x.FunctionalAreaProcessAssessments.Select(
                    p => p.ControlAssessments.Select(c => c.ControlType)))
                .Include(x => x.FunctionalAreaProcessAssessments.Select(
        ...
        private static void ResolveChildEntities(CustomDbContext context, Assessment entity)
        {
            ...
            ResolveEntities(context, entity.Products);
        ...
        private static void UpdateEntityFields(Assessment existing, Assessment newEntity, CustomDbContext context)
        {
            ...
            existing.Products = newEntity.Products;
        }
            ...
            where T : LookupEntity
        {
            ...
            foreach (ProcessControlAssessment pca in pcas)
            {
                CompositeBar compositeBar =
                    report.CompositeBars.FirstOrDefault(x => x.RiskType == riskTypeGetter(pca));
                CategoryBar categoryBar =
                    compositeBar?.CategoryBars.FirstOrDefault(x => x.Category.Id == pca.Category.Id);
                if (categoryBar != null)
                {
                    categoryBar.Value++;
                }
        ...
        private static void ExportBusinessUnitSheet(XLWorkbook wb, Assessment assessment, IList<string> sheetNames)
        {
            ...
            SetCells(ws, 3, 1, "Product", GetLookupCvs(assessment.Products));
        ...
        private static string GetSheetName(IList<string> sheetNames, string sheetName)
        {
            ...
            {
                int foundSheetNamesCount = sheetNames.Count(p => p == sheetName);
                correctSheetName = correctSheetName.Length + foundSheetNamesCount.ToString().Length <= 31
                    ? correctSheetName
