...
namespace JohnHancock.ProjectCoeus.Services.Impl
{
    ...
    public class CustomDbContext : DbContext
    {
        ...
        /// <summary>
        /// Gets or sets the user mapping info set.
        /// </summary>
        /// <value>
        /// The user mapping info set.
        /// </value>
        public DbSet<UserMappingInfo> UserMappingInfoSet { get; set; }

        /// <summary>
        /// Gets or sets the RiskScoreRange set.
        /// </summary>
        /// <value>
        /// The RiskScoreRange set.
        /// </value>
        public DbSet<RiskScoreRange> RiskScoreRangeSet { get; set; }
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Assessment>().HasMany(x => x.Products)
                .WithMany().Map(x =>
                {
                    x.MapLeftKey("AssessmentId");
                    x.MapRightKey("ProductId");
                    x.ToTable("Assessment_Product");
                });

            modelBuilder.Entity<UserMappingInfo>().HasMany(x => x.BusinessUnits)
                .WithMany().Map(x =>
                {
                    x.MapLeftKey("UserId");
                    x.MapRightKey("BusinessUnitId");
                    x.ToTable("User_BusinessUnit");
                });

            modelBuilder.Entity<KPICategory>().HasMany(x => x.SLAs)
                .WithMany().Map(x =>
                {
                    x.MapLeftKey("KPICategoryId");
                    x.MapRightKey("SLAId");
                    x.ToTable("KPICategory_SLA");
                });
